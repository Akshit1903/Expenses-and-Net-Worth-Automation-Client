...
class _ExpensesPageState extends State<ExpensesPage> {
  ...
  Future<void> _pickDocument(String type) async {
    ...
    if (result != null) {
      setState(() {
        switch (type) {
          case 'HDFC':
            _hdfcPath = result.files.single.path;
            break;
          case 'SBI CC':
            _sbiCcPath = result.files.single.path;
            break;
          case 'HDFC CC':
            _hdfcCcPath = result.files.single.path;
            break;
          case 'SBI':
            _sbiPath = result.files.single.path;
            break;
          case 'Payslip':
            _payslipPath = result.files.single.path;
            break;
        }
      });
    }
  ...
  String? _getFilePath(String type) {
     switch (type) {
          case 'HDFC': return _hdfcPath;
          case 'SBI CC': return _sbiCcPath;
          case 'HDFC CC': return _hdfcCcPath;
          case 'SBI': return _sbiPath;
          case 'Payslip': return _payslipPath;
          default: return null;
        }
  }
  ...
  Widget _buildFileUploadCard(String title, String type, IconData icon) {
    String? path = _getFilePath(type);
    bool isSelected = path != null;
    ...
    return Card(
      ...
      child: ListTile(
        leading: Icon(icon, color: isSelected ? Colors.green : Theme.of(context).colorScheme.primary),
        title: Text(title, style: const TextStyle(fontWeight: FontWeight.w500)),
        subtitle: Text(
          isSelected ? path.split(Platform.pathSeparator).last : 'Tap to upload',
          maxLines: 1,
        ...
        trailing: isSelected ? IconButton(
          ...
          onPressed: () {
            setState(() {
               switch (type) {
                case 'HDFC': _hdfcPath = null; break;
                case 'SBI CC': _sbiCcPath = null; break;
                case 'HDFC CC': _hdfcCcPath = null; break;
                case 'SBI': _sbiPath = null; break;
                case 'Payslip': _payslipPath = null; break;
              }
            });
          },
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          _buildFileUploadCard('HDFC Statement', 'HDFC', Icons.account_balance),
          const SizedBox(height: 8),
          _buildFileUploadCard('SBI CC Statement', 'SBI CC', Icons.credit_card),
          const SizedBox(height: 8),
          _buildFileUploadCard('HDFC CC Statement', 'HDFC CC', Icons.credit_card),
          const SizedBox(height: 8),
          _buildFileUploadCard('SBI Statement', 'SBI', Icons.account_balance),
          const SizedBox(height: 8),
          _buildFileUploadCard('Payslip', 'Payslip', Icons.receipt_long),

          const SizedBox(height: 24),

          // Results Section
          if (_createSpreadSheetByUploadingCSVFileResponse != '{}' || _appsScriptResponse != '{}')
             Column(
              crossAxisAlignment: CrossAxisAlignment.stretch,
              children: [
                Text(
                ...
                const SizedBox(height: 8),
                 if (_createSpreadSheetByUploadingCSVFileResponse != '{}') ...[
                   Text("Spreadsheet Response:", style: Theme.of(context).textTheme.labelLarge),
                   Container(height: 150, child: _getJsonWidget(_createSpreadSheetByUploadingCSVFileResponse)),
                   const SizedBox(height: 8),
                   OutlinedButton.icon(
                 ...
                 if (_appsScriptResponse != '{}') ...[
                   Text("Apps Script Response:", style: Theme.of(context).textTheme.labelLarge),
                   Container(height: 200, child: _getJsonWidget(_appsScriptResponse)),
                 ],
